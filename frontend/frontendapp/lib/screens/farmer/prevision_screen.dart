...
class _PrevisionScreenState extends State<PrevisionScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<PrevisionProvider>().fetchPrevision();
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return RefreshIndicator(
      onRefresh: () => context.read<PrevisionProvider>().fetchPrevision(),
            ...
            if (provider.isLoading && data == null)
              ...
              Padding(
                ...
                child: Column(
                  children: [
                    ...
                    PrimaryButton(
                      label: 'Réessayer',
                      onPressed: () => context.read<PrevisionProvider>().fetchPrevision(),
                    ),
                  ],
                ),
              )
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: (data?.besoinEau ?? false) ? AppColors.blue : AppColors.green,
                  borderRadius: BorderRadius.circular(16),
                ),
                child: Column(
                  children: [
                    Icon(
                      (data?.besoinEau ?? false) ? Icons.water_drop : Icons.check_circle_outline,
                      color: Colors.white,
                      size: 30,
                    ),
                            ...
                            child: Container(
                              ...
                              child: Column(
                                children: [
                                  ...
                                  Text(
                                    '${p.valeur.toStringAsFixed(0)}%',
                                    style: const TextStyle(fontSize: 12.5, fontWeight: FontWeight.w700, color: AppColors.greenDark),
                                  ),
                                ],
                              ),
                            ),
